...
SELECT market_date, total_sales, 'Best Day' AS best_or_worst
FROM ranked_sales
WHERE best_rank = 1

UNION

SELECT market_date, total_sales, 'Worst Day' AS best_or_worst
FROM ranked_sales
WHERE worst_rank = 1;
...
v.vendor_name,
p.product_name,
vi.original_price * 5 * COUNT(c.customer_id) AS total_revenue
FROM
vendor AS v
INNER JOIN
...
VALUES (24,'Apple Pie','large',17,'unit', CURRENT_TIMESTAMP);
